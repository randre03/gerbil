...
(def c3-test
  (test-suite "test :gerbil/runtime/c3"
    (test-case "c3 linearization"
      ...
      (check (map ##type-name (type-descriptor-mixin Z::t)) => '(K1 K2 K3 D A B C E O)) ;; FIXED!
      (check (map ##type-name (type-descriptor-mixin Y::t)) => '(J1 C J3 A J2 B D E O)) ;; same!

      ;; Slot computation order now follows the MRO!
      ;; Previously returned (O A B C K1 D E K2 K3 Z), which is so wrong:
      (check (cdr (assq slots: (type-descriptor-plist Z::t))) => '(O E C B A D K3 K2 K1 Z))
