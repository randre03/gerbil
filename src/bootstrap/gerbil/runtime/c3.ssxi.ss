...
(begin
  ...
  (declare-type remove-next__% (@lambda 3 #f))
  (declare-type remove-next__0 (@lambda 2 #f))
  (declare-type
   remove-next
   (@case-lambda (2 remove-next__0) (3 remove-next__%)))
  (declare-type not-null? (@lambda 1 #f))
  (declare-type remove-nulls (@lambda 1 #f))
  (declare-type append-reverse (@lambda 2 #f))
  (declare-type every/1 (@lambda 2 #f)))
